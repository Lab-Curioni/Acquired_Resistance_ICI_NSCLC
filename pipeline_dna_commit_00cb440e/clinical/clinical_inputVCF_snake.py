...
rule clinical_inputVCF:
    input:
	    expand(DATABASEQUERY + '{sample}.snpEff.dbSNP.clinVar.cosmic.dbnsfp.pass.overview.txt', sample = getSampleNames()),
	    expand(DATABASEQUERY + '{sample}.snpEff.dbSNP.clinVar.cosmic.dbnsfp.pass.overview.dgidb.txt.CompleteTable.txt', sample = getSampleNames()),
	    expand(DATABASEQUERY + '{sample}.snpEff.dbSNP.clinVar.cosmic.dbnsfp.pass.overview.dgidb.txt.CompleteTable.ClinicalTrials.txt', sample = getSampleNames()),
	    expand(DATABASEQUERY + '{sample}.snpEff.dbSNP.clinVar.cosmic.dbnsfp.pass.overview.databaseQueriesSNV.txt_dgidbIndependent.txt_damaging.txt', sample = getSampleNames())
